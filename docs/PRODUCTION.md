...
В репозитории есть generic production-путь через `lego`. Укажите DNS provider
из списка lego и credentials, которые требует этот provider.

Для Beget это выглядит так:

```dotenv
LEGO_DNS_PROVIDER=beget
LEGO_IMAGE=goacme/lego:v4.27.0
